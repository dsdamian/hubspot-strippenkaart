...
exports.main = async (context = {}) => {
  try {
    const { companyId, uren, werktype, omschrijving, datum, medewerker } =
      context.parameters || {};
    if (!companyId) return { error: 'companyId ontbreekt' };
    const aantal = num(uren);
    if (aantal <= 0) return { error: 'Aantal uren moet groter dan 0 zijn' };

    const types = await getTypeIds();
    const data = await getData(String(companyId));
    const kaart = data.actieve;
    if (!kaart) {
      return { error: 'Geen actieve strippenkaart voor dit bedrijf. Maak er eerst een aan.' };
    }

    const reg = await api(`/crm/v3/objects/${types.uren}`, 'POST', {
      properties: {
        omschrijving: omschrijving || '',
        datum: datumNaarMs(datum),
        uren: aantal,
        werktype: werktype || 'overig',
        medewerker: medewerker || '',
      },
    });

    await associateDefault(types.uren, reg.id, 'companies', String(companyId));
    await associateDefault(types.uren, reg.id, types.strip, kaart.id);

    const nieuwVerbruikt = kaart.verbruikt + aantal;
