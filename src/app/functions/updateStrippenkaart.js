...
exports.main = async (context = {}) => {
  try {
    const { companyId, kaartId, naam, totaalUren, uurtarief, startdatum } =
      context.parameters || {};
    if (!companyId || !kaartId) return { error: 'companyId of kaartId ontbreekt' };
    const totaal = num(totaalUren);
    if (totaal <= 0) return { error: 'Totaal uren moet groter dan 0 zijn' };

    const types = await getTypeIds();
    const kaart = (await batchRead(types.strip, [String(kaartId)], STRIP_PROPS)).map(mapKaart)[0];
    if (!kaart) return { error: 'Strippenkaart niet gevonden' };

    await api(`/crm/v3/objects/${types.strip}/${kaartId}`, 'PATCH', {
      properties: {
        naam: naam || kaart.naam,
        totaal_uren: totaal,
        uurtarief: num(uurtarief),
        startdatum: datumNaarMs(startdatum),
        resterende_uren: totaal - kaart.verbruikt,
        status: berekenStatus(totaal, kaart.verbruikt, kaart.geldigTot),
      },
    });
