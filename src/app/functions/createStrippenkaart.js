...
exports.main = async (context = {}) => {
  try {
    const { companyId, naam, totaalUren, uurtarief } = context.parameters || {};
    if (!companyId) return { error: 'companyId ontbreekt' };
    const totaal = num(totaalUren);
    if (totaal <= 0) return { error: 'Aantal uren moet groter dan 0 zijn' };

    const types = await getTypeIds();
    const s = context.parameters && context.parameters.startdatum;
    let startdatum;
    if (s && typeof s === 'object' && s.year != null) {
      startdatum = Date.UTC(s.year, s.month, s.date);
    } else {
      const nu = new Date();
      startdatum = Date.UTC(nu.getUTCFullYear(), nu.getUTCMonth(), nu.getUTCDate());
    }

    const kaart = await api(`/crm/v3/objects/${types.strip}`, 'POST', {
      properties: {
        naam: naam || `HubSpot Strippenkaart ${totaal} uur`,
        totaal_uren: totaal,
        verbruikte_uren: 0,
        resterende_uren: totaal,
    ...
    await associateDefault(types.strip, kaart.id, 'companies', String(companyId));
